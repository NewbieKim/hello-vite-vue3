module.exports = {
  ...
  rules: {
    ...
    "no-caller": 0,//禁止使用arguments.caller或arguments.callee --不生效
    ...
    // "no-empty-label": 2,//禁止使用空label,此规则已在 ESLint v2.0 中删除
    ...
    "no-inline-comments": 0,//禁止行内备注 --不生效
    "no-inner-declarations": [2, "functions"],//禁止在块语句中使用声明（变量或函数）
    "no-invalid-regexp": 2,//禁止无效的正则表达式
    "no-invalid-this": 0,//禁止无效的this，只能用在构造器，类，对象字面量 -不生效
    ...
    "no-plusplus": 0,//禁止使用++，--
    "no-process-env": 'error',//禁止使用process.env
    ...
    "no-unused-vars": [0, {"vars": "all", "args": "after-used","argsIgnorePattern": "^E"}],//不能有声明后未被使用的变量或参数----灰色提示替代
    ...
    "eol-last": 'error',//文件以单一的换行符结束--强制执行VUE特性
    "eqeqeq": 2,//必须使用全等
    ...
    "max-nested-callbacks": ['error', 2],//回调嵌套深度
    "max-params": ['error', 5],//函数最多只能有3个参数
    // "max-statements": ['error', 10],//函数内最多有几个声明--请尽量原子化业务
    'vue/match-component-file-name': 'error',
    "new-cap": 2,//函数名首行大写必须使用new方式调用，首行小写必须用不带new方式调用
    "new-parens": 2,//new时必须加小括号
    "newline-after-var": 0,//变量声明后是否需要空一行,不执行
    ...
    "semi": [2, "never"],//语句强制分号结尾-强制不使用
    // "semi-spacing": [0, {"before": false, "after": true}],//分号前后空格-不使用此规则
    "sort-vars": 0,//变量声明时排序
    ...
    // "space-return-throw-case": 2,//return throw case后面要不要加空格,此规则在ESLint v2.0中已删除
    ...
    "vue/max-attributes-per-line": ["error", {"singleline": 6, "multiline": { "max": 1,"allowFirstLine": false }}],//单行attribute最多6个 超过换行每行一个
  },
  'extends': [
    'eslint:recommended',
